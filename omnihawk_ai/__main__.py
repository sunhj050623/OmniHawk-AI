...
from omnihawk_ai.context import AppContext
from omnihawk_ai import __version__
from omnihawk_ai.core import load_config, parse_multi_account_config, validate_paired_configs
from omnihawk_ai.core.analyzer import convert_keyword_stats_to_platform_stats
from omnihawk_ai.crawler import DataFetcher
from omnihawk_ai.storage import convert_crawl_results_to_news_data
from omnihawk_ai.utils.time import DEFAULT_TIMEZONE, is_within_days, calculate_days_old
from omnihawk_ai.ai import AIAnalyzer, AIAnalysisResult
from omnihawk_ai.core.scheduler import ResolvedSchedule
...
class NewsAnalyzer:
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        if config is None:
            print("正在加载配置...")
            config = load_config()
        print(f"OmniHawk AI v{__version__} 配置加载完成")
    ...
    def _crawl_rss_data(self) -> Tuple[Optional[List[Dict]], Optional[List[Dict]], Optional[List[Dict]], set]:
        ...
        try:
            from omnihawk_ai.crawler.rss import RSSFetcher, RSSFeedConfig
    ...
    def _process_rss_data_by_mode(self, rss_data) -> Tuple[Optional[List[Dict]], Optional[List[Dict]], Optional[List[Dict]], set]:
        ...
        """
        from omnihawk_ai.core.analyzer import count_rss_frequency
    ...
    def _filter_rss_by_keywords(self, rss_items: List[Dict]) -> List[Dict]:
        ...
        try:
            ...
            if word_groups or filter_words or global_filters:
                from omnihawk_ai.core.frequency import matches_word_groups
                filtered_items = []
                for item in rss_items:
                    title = item.get("title", "")
    ...
    def _generate_rss_html_report(self, rss_items: list, feeds_info: dict) -> str:
        """生成 RSS HTML 报告"""
        try:
            from omnihawk_ai.report.rss_html import render_rss_html_content
            from pathlib import Path
...
def _run_doctor(config_path: Optional[str] = None) -> bool:
    """运行环境体检"""
    print("=" * 60)
    print(f"OmniHawk AI v{__version__} 环境体检")
    print("=" * 60)
...
def _build_test_report_data(ctx: AppContext) -> Dict:
    ...
    title = f"OmniHawk AI 通知测试消息（{now.strftime('%Y-%m-%d %H:%M:%S')}）"
        ...
        "stats": [
            {
                ...
                "titles": [
                    {
                        "title": title,
                        "source_name": "OmniHawk AI",
                        "url": "https://github.com/sansan0/OmniHawk AI",
                        "mobile_url": "",
                        "ranks": [1],
...
<html lang="zh-CN">
<head><meta charset="UTF-8"><title>OmniHawk AI 通知测试</title></head>
<body>
<h2>OmniHawk AI 通知连通性测试</h2>
...
def _run_test_notification(config: Dict) -> bool:
    """发送测试通知到已配置渠道"""
    from omnihawk_ai.notification import NotificationDispatcher
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="OmniHawk AI - 热点新闻聚合与分析工具",
...
示例:
  python -m omnihawk_ai                    # 正常运行
  python -m omnihawk_ai --show-schedule    # 查看当前调度状态
  python -m omnihawk_ai --doctor           # 运行一键体检
  python -m omnihawk_ai --test-notification # 测试通知渠道连通性
"""
    )
    parser.add_argument(
        "--show-schedule",
...
def _handle_status_commands(config: Dict) -> None:
    ...
    print("=" * 60)
    print(f"OmniHawk AI v{__version__} 调度状态")
    print("=" * 60)
