...
from omnihawk_ai.report.formatter import format_title_for_platform
from omnihawk_ai.report.helpers import format_rank_display
from omnihawk_ai.utils.time import DEFAULT_TIMEZONE, format_iso_time_friendly, convert_time_for_display
from omnihawk_ai.notification.batch import truncate_at_line_boundary
...
def split_content_into_batches(
    ...
    if format_type in ("wework", "bark"):
        ...
        if update_info:
            base_footer += f"\n> OmniHawk AI 发现新版本 **{update_info['remote_version']}**，当前 **{update_info['current_version']}**"
    elif format_type == "telegram":
        base_footer = f"\n\n更新时间：{now.strftime('%Y-%m-%d %H:%M:%S')}"
        if update_info:
            base_footer += f"\nOmniHawk AI 发现新版本 {update_info['remote_version']}，当前 {update_info['current_version']}"
        ...
        if update_info:
            base_footer += f"\n<font color='grey'>OmniHawk AI 发现新版本 {update_info['remote_version']}，当前 {update_info['current_version']}</font>"
    elif format_type == "dingtalk":
        base_footer = f"\n\n> 更新时间：{now.strftime('%Y-%m-%d %H:%M:%S')}"
        if update_info:
            base_footer += f"\n> OmniHawk AI 发现新版本 **{update_info['remote_version']}**，当前 **{update_info['current_version']}**"
        ...
        if update_info:
            base_footer += f"\n_OmniHawk AI 发现新版本 *{update_info['remote_version']}*，当前 *{update_info['current_version']}_"
