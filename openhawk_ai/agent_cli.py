...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="OpenHawk Agent CLI (MCP-compatible local tool invoker)",
        ...
        epilog=(
            ...
            "  openhawk-ai-cli call set_schedule_interval --args '{\"interval_minutes\":20}'\n"
            "  openhawk-ai-cli call set_schedule_cron --args '{\"cron_expr\":\"*/15 * * * *\"}'\n"
            "  openhawk-ai-cli --project-root . --output-dir ./output call list_papers --args '{\"limit\":5}'\n"
        ),
    )
    parser.add_argument("--project-root", default="", help="Override OpenHawk project root path")
            ...
            _dump(
                {
                    ...
                    "hint": "run `openhawk-ai-cli tools` to list available tools",
                },
                compact=compact,
            )
        )
            ...
            _dump(
                {
                    ...
                    "hint": "run `openhawk-ai-cli tools` and check this tool's params",
                },
                compact=compact,
            )
        )
