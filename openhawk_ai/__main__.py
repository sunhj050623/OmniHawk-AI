...
from openhawk_ai.context import AppContext
from openhawk_ai import __version__
from openhawk_ai.core import load_config, parse_multi_account_config, validate_paired_configs
from openhawk_ai.core.analyzer import convert_keyword_stats_to_platform_stats
from openhawk_ai.crawler import DataFetcher
from openhawk_ai.storage import convert_crawl_results_to_news_data
from openhawk_ai.utils.time import DEFAULT_TIMEZONE, is_within_days, calculate_days_old
from openhawk_ai.ai import AIAnalyzer, AIAnalysisResult
from openhawk_ai.core.scheduler import ResolvedSchedule
...
class NewsAnalyzer:
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        if config is None:
            print("正在加载配置...")
            config = load_config()
        print(f"OpenHawk v{__version__} 配置加载完成")
    ...
    def _crawl_rss_data(self) -> Tuple[Optional[List[Dict]], Optional[List[Dict]], Optional[List[Dict]], set]:
        ...
        try:
            from openhawk_ai.crawler.rss import RSSFetcher, RSSFeedConfig
            ...
            # Reuse cached paper insights from local storage to avoid repeated LLM analysis.
            try:
                existing_rss = self.storage_manager.get_latest_rss_data()
            except Exception:
                existing_rss = None
            if existing_rss and isinstance(getattr(existing_rss, "items", None), dict):
                cached_by_arxiv: Dict[str, Dict] = {}
                cached_by_url: Dict[str, Dict] = {}
                for _, existing_list in existing_rss.items.items():
                    for existing_item in existing_list:
                        if not isinstance(existing_item.paper_insight, dict) or not existing_item.paper_insight:
                            continue
                        existing_meta = existing_item.paper_meta if isinstance(existing_item.paper_meta, dict) else {}
                        arxiv_id = str(existing_meta.get("arxiv_id", "") or "").strip().lower()
                        url = str(existing_item.url or "").strip()
                        if arxiv_id and arxiv_id not in cached_by_arxiv:
                            cached_by_arxiv[arxiv_id] = dict(existing_item.paper_insight)
                        if url and url not in cached_by_url:
                            cached_by_url[url] = dict(existing_item.paper_insight)

                reused_count = 0
                for _, rss_list in rss_data.items.items():
                    for item in rss_list:
                        if isinstance(item.paper_insight, dict) and item.paper_insight:
                            continue
                        meta = item.paper_meta if isinstance(item.paper_meta, dict) else {}
                        arxiv_id = str(meta.get("arxiv_id", "") or "").strip().lower()
                        url = str(item.url or "").strip()
                        cached = cached_by_arxiv.get(arxiv_id) if arxiv_id else None
                        if not cached and url:
                            cached = cached_by_url.get(url)
                        if isinstance(cached, dict) and cached:
                            item.paper_insight = dict(cached)
                            reused_count += 1
                if reused_count > 0:
                    print(f"[Paper] reused cached insights for {reused_count} items before analysis")

            # Enrich arXiv RSS items with paper insights before persistence.
            paper_cfg = self.ctx.config.get("PAPER_ANALYSIS", {})
            if isinstance(paper_cfg, dict) and bool(paper_cfg.get("ENABLED", False)):
                try:
                    from openhawk_ai.ai.paper_analyzer import PaperAnalyzer

                    ai_cfg = self.ctx.config.get("AI", {})
                    analyzer = PaperAnalyzer(ai_cfg if isinstance(ai_cfg, dict) else {}, paper_cfg)
                    analyzer.enrich_rss_data(rss_data)
                except Exception as exc:
                    print(f"[Paper] analyzer init/run failed: {type(exc).__name__}: {exc}")
    ...
    def _process_rss_data_by_mode(self, rss_data) -> Tuple[Optional[List[Dict]], Optional[List[Dict]], Optional[List[Dict]], set]:
        ...
        """
        from openhawk_ai.core.analyzer import count_rss_frequency
    ...
    def _filter_rss_by_keywords(self, rss_items: List[Dict]) -> List[Dict]:
        ...
        try:
            ...
            if word_groups or filter_words or global_filters:
                from openhawk_ai.core.frequency import matches_word_groups
                filtered_items = []
                for item in rss_items:
                    title = item.get("title", "")
    ...
    def _generate_rss_html_report(self, rss_items: list, feeds_info: dict) -> str:
        """生成 RSS HTML 报告"""
        try:
            from openhawk_ai.report.rss_html import render_rss_html_content
            from pathlib import Path
...
def _run_doctor(config_path: Optional[str] = None) -> bool:
    """运行环境体检"""
    print("=" * 60)
    print(f"OpenHawk v{__version__} 环境体检")
    print("=" * 60)
...
def _build_test_report_data(ctx: AppContext) -> Dict:
    ...
    title = f"OpenHawk 通知测试消息（{now.strftime('%Y-%m-%d %H:%M:%S')}）"
        ...
        "stats": [
            {
                ...
                "titles": [
                    {
                        "title": title,
                        "source_name": "OpenHawk",
                        "url": "https://github.com/sansan0/OpenHawk",
                        "mobile_url": "",
                        "ranks": [1],
...
<html lang="zh-CN">
<head><meta charset="UTF-8"><title>OpenHawk 通知测试</title></head>
<body>
<h2>OpenHawk 通知连通性测试</h2>
...
def _run_test_notification(config: Dict) -> bool:
    """发送测试通知到已配置渠道"""
    from openhawk_ai.notification import NotificationDispatcher
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="OpenHawk - 热点新闻聚合与分析工具",
...
示例:
  python -m openhawk_ai                    # 正常运行
  python -m openhawk_ai --show-schedule    # 查看当前调度状态
  python -m openhawk_ai --doctor           # 运行一键体检
  python -m openhawk_ai --test-notification # 测试通知渠道连通性
"""
    )
    parser.add_argument(
        "--show-schedule",
...
def _handle_status_commands(config: Dict) -> None:
    ...
    print("=" * 60)
    print(f"OpenHawk v{__version__} 调度状态")
    print("=" * 60)
