...
from openhawk_ai.utils.time import DEFAULT_TIMEZONE
...
def _load_crawler_config(config_data: Dict) -> Dict:
    ...
    return {
        ...
        "ENABLE_CRAWLER": enable_crawler_env if enable_crawler_env is not None else platforms_config.get("enabled", True),
    }
...
def _load_notification_config(config_data: Dict) -> Dict:
    ...
    if enabled_env is None:
        enabled_env = _get_env_bool("NOTIFICATION_ENABLED")
...
def _load_paper_analysis_config(config_data: Dict) -> Dict:
    """加载论文分析配置（面向 arXiv RSS 条目）"""
    paper_config = config_data.get("paper_analysis", {})

    enabled_env = _get_env_bool("PAPER_ANALYSIS_ENABLED")
    analyze_all_new_env = _get_env_bool("PAPER_ANALYSIS_ANALYZE_ALL_NEW")
    timeout_env = _get_env_int_or_none("PAPER_ANALYSIS_TIMEOUT")
    max_tokens_env = _get_env_int_or_none("PAPER_ANALYSIS_MAX_TOKENS")
    max_papers_env = _get_env_int_or_none("PAPER_ANALYSIS_MAX_PAPERS_PER_RUN")
    language_env = _get_env_str("PAPER_ANALYSIS_LANGUAGE")

    return {
        "ENABLED": enabled_env if enabled_env is not None else paper_config.get("enabled", False),
        "ANALYZE_ALL_NEW": (
            analyze_all_new_env
            if analyze_all_new_env is not None
            else paper_config.get("analyze_all_new", True)
        ),
        "PROMPT_FILE": paper_config.get("prompt_file", "paper_analysis_prompt.txt"),
        "MAX_PAPERS_PER_RUN": max_papers_env if max_papers_env is not None else paper_config.get("max_papers_per_run", 8),
        "MAX_ABSTRACT_CHARS": paper_config.get("max_abstract_chars", 3000),
        "TIMEOUT": timeout_env if timeout_env is not None else paper_config.get("timeout", 120),
        "MAX_TOKENS": max_tokens_env if max_tokens_env is not None else paper_config.get("max_tokens", 1200),
        "LANGUAGE": language_env or paper_config.get("language", "Chinese"),
        "RESEARCH_TOPIC": _get_env_str("PAPER_RESEARCH_TOPIC") or paper_config.get("research_topic", ""),
        "PRIMARY_CATEGORY": _get_env_str("PAPER_PRIMARY_CATEGORY") or paper_config.get("primary_category", ""),
        "SUBTOPICS": _get_env_str("PAPER_SUBTOPICS") or paper_config.get("subtopics", ""),
    }
...
def load_config(config_path: Optional[str] = None) -> Dict[str, Any]:
    ...
    config["PAPER_ANALYSIS"] = _load_paper_analysis_config(config_data)
