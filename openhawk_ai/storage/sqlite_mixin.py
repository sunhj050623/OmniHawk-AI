...
from openhawk_ai.storage.base import NewsItem, NewsData, RSSItem, RSSData
from openhawk_ai.utils.url import normalize_url
...
class SQLiteStorageMixin:
    ...
    def _init_tables(self, conn: sqlite3.Connection, db_type: str = "news") -> None:
        ...
        if db_type == "news":
            ...
            self._ensure_rss_schema_columns(conn)

        conn.commit()

    def _ensure_rss_schema_columns(self, conn: sqlite3.Connection) -> None:
        """Ensure rss_items contains paper fields for older databases."""
        cursor = conn.cursor()
        cursor.execute("PRAGMA table_info(rss_items)")
        existing_columns = {row[1] for row in cursor.fetchall()}

        if "paper_meta_json" not in existing_columns:
            cursor.execute("ALTER TABLE rss_items ADD COLUMN paper_meta_json TEXT DEFAULT ''")
        if "paper_insight_json" not in existing_columns:
            cursor.execute("ALTER TABLE rss_items ADD COLUMN paper_insight_json TEXT DEFAULT ''")

    def _safe_load_json_dict(self, raw_value: Any) -> Dict[str, Any]:
        """Safely parse JSON text to dict, fallback to empty dict."""
        if not raw_value:
            return {}
    ...
    def _save_rss_data_impl(self, data: RSSData, log_prefix: str = "[??]") -> tuple[bool, int, int]:
        """
        ?? RSS ??? SQLite??????
        ...
        try:
            ...
            for feed_id, rss_list in data.items.items():
                for item in rss_list:
                    try:
                        paper_meta_json = json.dumps(item.paper_meta or {}, ensure_ascii=False) if item.paper_meta else ""
                        paper_insight_json = json.dumps(item.paper_insight or {}, ensure_ascii=False) if item.paper_insight else ""
                            ...
                            if existing:
                                ...
                                cursor.execute("""
                                    UPDATE rss_items SET
                                        ...
                                        author = ?,
                                        paper_meta_json = ?,
                                        paper_insight_json = CASE
                                            WHEN ? <> '' THEN ?
                                            ELSE paper_insight_json
                                        END,
                                        last_crawl_time = ?,
                                        crawl_count = crawl_count + 1,
                                        updated_at = ?
                                ...
                                cursor.execute("""
                                    ...
                                    (title, feed_id, url, published_at, summary, author, paper_meta_json, paper_insight_json,
                                     first_crawl_time, last_crawl_time, crawl_count,
                                     created_at, updated_at)
                                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1, ?, ?)
                                    ON CONFLICT(url, feed_id) DO UPDATE SET
                                        ...
                                        paper_meta_json = excluded.paper_meta_json,
                                        paper_insight_json = CASE
                                            WHEN excluded.paper_insight_json <> '' THEN excluded.paper_insight_json
                                            ELSE rss_items.paper_insight_json
                                        END,
                        ...
                        print(f"{log_prefix} ?? RSS ???? [{item.title[:30]}...]: {e}")
        ...
        try:
            ...
            cursor.execute("""
                SELECT i.id, i.title, i.feed_id, f.name as feed_name,
                       ...
                       i.paper_meta_json, i.paper_insight_json
                FROM rss_items i
            ...
            for row in rows:
                ...
                paper_meta = self._safe_load_json_dict(row[11])
                paper_insight = self._safe_load_json_dict(row[12])
                    ...
                    for fr_row in rss_filter_rows:
                        ...
                        if info:
                            results.append({
                                ...
                                "summary": info[6] or "",
                                "author": info[7] or "",
                                "paper_meta": self._safe_load_json_dict(info[8]),
                                "paper_insight": self._safe_load_json_dict(info[9]),
                            })
            except Exception:
                pass  # RSS 库不存在时静默跳过
    ...
    def _get_all_rss_ids_impl(self, date: Optional[str] = None) -> List[Dict]:
        ...
        try:
            ...
            cursor.execute("""
                SELECT i.id, i.title, i.feed_id, f.name as feed_name,
                       i.published_at, i.summary, i.paper_meta_json, i.paper_insight_json
                FROM rss_items i
                LEFT JOIN rss_feeds f ON i.feed_id = f.id
                ORDER BY i.id
            """)
            ...
            return [
                {
                    ...
                    "summary": row[5] or "",
                    "paper_meta": self._safe_load_json_dict(row[6]),
                    "paper_insight": self._safe_load_json_dict(row[7]),
                }
                for row in cursor.fetchall()
            ]
        except Exception as e:
