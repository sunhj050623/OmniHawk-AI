...
def _render_ai_analysis(ai_analysis: Any, channel: str) -> str:
    ...
    try:
        from openhawk_ai.ai.formatter import get_ai_analysis_renderer
    ...
    *,
    ...
    for i, batch_content in enumerate(batches, 1):
        ...
        payload = {
            "msgtype": "markdown",
            "markdown": {
                "title": f"OpenHawk 热点分析报告 - {report_type}",
                "text": batch_content,
            },
        }
...
def send_to_email(
    ...
    *,
    ...
    try:
        ...
        subject = f"OpenHawk 热点分析报告 - {report_type} - {now.strftime('%m月%d日 %H:%M')}"
