# coding=utf-8
"""
Interactive paper dashboard server for openhawk_ai.

Features:
- Manual trigger for `python -m openhawk_ai`
...
from openhawk_ai.translation import FastTranslator
from openhawk_ai.web.panel_templates import (
...
class CrawlRunner:
    ...
    def trigger(self, analysis_language: str = "") -> Tuple[bool, str]:
        with self._lock:
            ...
            # Panel mode expects paper insights to be produced for the paper page.
            # Force-enable paper analysis regardless of base config defaults.
            env["PAPER_ANALYSIS_ENABLED"] = "true"
            env["PAPER_ANALYSIS_ANALYZE_ALL_NEW"] = "true"
    ...
    def _detect_existing_crawl_pid(self) -> Optional[int]:
        """
        Detect external `python -m openhawk_ai` process to avoid duplicate runs.
        ...
        for entry in proc_root.iterdir():
            ...
            if "openhawk_ai.web.panel_server" in cmdline:
                continue
            if "-m openhawk_ai" in cmdline:
                return pid
...
class ScheduleController:
    ...
    def _build_command_from_settings(self) -> str:
        ...
        env_parts: List[str] = [
            ...
            "PAPER_ANALYSIS_ENABLED=true",
            "PAPER_ANALYSIS_ANALYZE_ALL_NEW=true",
        ]
        mapping = {
        ...
        if env_parts:
            return f"cd /app && {' '.join(env_parts)} {python_cmd} -m openhawk_ai"
        return f"cd /app && {python_cmd} -m openhawk_ai"
    ...
    def _fetch_affiliations_from_arxiv(self, arxiv_id: str) -> List[str]:
        ...
        try:
            ...
            req = urllib.request.Request(
                url,
                headers={"User-Agent": "OpenHawk-Panel/1.0"},
            )
    ...
    def list_papers(
        ...
        for row in rows:
            ...
            if not analyzed:
                ...
                continue
            if view_mode == "all" and selected_category:
                known_categories = {x for x in [primary_category] + tags if str(x or "").strip()}
                if known_categories and selected_category not in known_categories:
    ...
    def _download_url_text(self, url: str, timeout: int = 10) -> str:
        ...
        try:
            req = urllib.request.Request(
                target,
                headers={
                    "User-Agent": "OpenHawk-Panel/1.0 (+https://arxiv.org)",
...
class DashboardHandler(BaseHTTPRequestHandler):
    ...
    monitor_html: str = build_monitor_html()
    ...
    @staticmethod
    def _monitor_duration_seconds(start_text: Any, end_text: Any) -> Optional[int]:
        started = parse_iso_utc(start_text)
        ended = parse_iso_utc(end_text)
    ...
    @staticmethod
    def _monitor_latest_iso(values: List[Any]) -> str:
        best_text = ""
        ...
        for raw in values:
            ...
            if dt is None:
                if not best_dt and text > best_text:
                    best_text = text
                continue
    ...
    @staticmethod
    def _monitor_file_info(path: Path) -> Dict[str, Any]:
        target = Path(path)
        ...
        try:
            ...
            age_seconds = int(max(0.0, (datetime.now(timezone.utc) - mtime_dt).total_seconds()))
            return {
                "path": str(target),
                "exists": True,
    ...
    def _monitor_collect_storage(self) -> Dict[str, Any]:
        output_dir = self.output_dir if isinstance(self.output_dir, Path) else Path("output")

        def latest_file(dir_path: Path, pattern: str) -> Optional[Path]:
            if not dir_path.exists() or not dir_path.is_dir():
                return None
            matched = [x for x in dir_path.glob(pattern) if x.is_file()]
            if not matched:
                return None
            matched.sort(key=lambda x: x.stat().st_mtime, reverse=True)
            return matched[0]

        news_latest = latest_file(output_dir / "news", "*.db")
        rss_latest = latest_file(output_dir / "rss", "*.db")
        progress_json = output_dir / "ai_progress_items.json"
        schedule_crontab = (
            self.schedule.crontab_path
            if self.schedule and isinstance(self.schedule.crontab_path, Path)
            else (output_dir / "panel_schedule.crontab")
        )

        return {
            "news_db_latest": self._monitor_file_info(news_latest or (output_dir / "news" / "_missing.db")),
            "rss_db_latest": self._monitor_file_info(rss_latest or (output_dir / "rss" / "_missing.db")),
            "progress_json": self._monitor_file_info(progress_json),
            "schedule_crontab": self._monitor_file_info(schedule_crontab),
        }

    def _monitor_collect_progress_snapshot(self) -> Dict[str, Dict[str, Any]]:
        loaded = self.progress.load() if self.progress else {}
        items = loaded.get("items") if isinstance(loaded, dict) else []
        if not isinstance(items, list):
            items = []
        updated_at = str(loaded.get("updated_at", "") or "").strip() if isinstance(loaded, dict) else ""

        scope_source_sets: Dict[str, set] = {scope: set() for scope in PROGRESS_SCOPES}
        scope_totals: Dict[str, int] = {scope: 0 for scope in PROGRESS_SCOPES}
        ...
        for row in items:
            ...
            if kind in {"official_blog", "official_site"}:
                scope = "frontier"
            elif kind in PROGRESS_SCOPE_KIND_MAP:
                scope = kind
            else:
                continue
            scope_totals[scope] = int(scope_totals.get(scope, 0)) + 1
            source_id = str(row.get("source_id", "") or "").strip()
            if source_id:
                scope_source_sets.setdefault(scope, set()).add(source_id)
        ...
        for scope in PROGRESS_SCOPES:
            output[scope] = {
                ...
                "total_items": int(scope_totals.get(scope, 0) or 0),
                "source_count": len(scope_source_sets.get(scope, set())),
                "updated_at": updated_at,
            }
        return output

    def _monitor_collect_paper_inventory(self, db_path: str) -> Dict[str, int]:
        path_text = str(db_path or "").strip()
        if not path_text:
            return {"total_items": 0, "source_count": 0}
        ...
        try:
            ...
            try:
                ...
                total_items = int(total_row[0]) if total_row and total_row[0] is not None else 0
                ...
                row = cur.fetchone()
                source_count = int(row[0]) if row and row[0] is not None else 0
                return {
    ...
    def _monitor_collect_push_quality(self) -> Dict[str, Any]:
        paper_payload = self.subscriptions.load() if self.subscriptions else {"items": []}
        paper_items = paper_payload.get("items") if isinstance(paper_payload, dict) else []
        if not isinstance(paper_items, list):
            paper_items = []
        paper_total = len([x for x in paper_items if isinstance(x, dict)])
        paper_enabled = len([x for x in paper_items if isinstance(x, dict) and bool(x.get("enabled", True))])
        paper_last_notified = self._monitor_latest_iso(
            [x.get("last_notified_at", "") for x in paper_items if isinstance(x, dict)]
        )

        progress_payload = self.progress_subscriptions.load() if self.progress_subscriptions else {"items": []}
        progress_items = progress_payload.get("items") if isinstance(progress_payload, dict) else []
        if not isinstance(progress_items, list):
            progress_items = []
        progress_total = len([x for x in progress_items if isinstance(x, dict)])
        progress_enabled = len([x for x in progress_items if isinstance(x, dict) and bool(x.get("enabled", True))])
        progress_last_notified = self._monitor_latest_iso(
            [x.get("last_notified_at", "") for x in progress_items if isinstance(x, dict)]
        )
        ...
        for scope in PROGRESS_SCOPES:
            scoped = [
                x
                for x in progress_items
                if isinstance(x, dict)
                and normalize_progress_scope(x.get("scope", "frontier"), default="frontier") == scope
            ]
            by_scope[scope] = {
                "scope": scope,
                "name_zh": PROGRESS_SCOPE_NAME_MAP_ZH.get(scope, scope),
                "total": len(scoped),
                "enabled": len([x for x in scoped if bool(x.get("enabled", True))]),
                "last_notified_at": self._monitor_latest_iso([x.get("last_notified_at", "") for x in scoped]),
            }

        settings_payload = self.progress_page_settings.load() if self.progress_page_settings else {"scopes": {}}
        scopes_cfg = settings_payload.get("scopes") if isinstance(settings_payload, dict) else {}
        if not isinstance(scopes_cfg, dict):
            scopes_cfg = {}
        auto_push_health: Dict[str, Dict[str, Any]] = {}
        for scope in PROGRESS_SCOPES:
            cfg = scopes_cfg.get(scope) if isinstance(scopes_cfg.get(scope), dict) else {}
            auto_push_health[scope] = {
                "scope": scope,
                "name_zh": PROGRESS_SCOPE_NAME_MAP_ZH.get(scope, scope),
                "auto_enabled": bool(parse_bool_text(cfg.get("auto_enabled"), False)),
                "auto_push_enabled": bool(parse_bool_text(cfg.get("auto_push_enabled"), False)),
                "auto_interval_minutes": parse_int_value(
                    cfg.get("auto_interval_minutes"),
                    PROGRESS_AUTO_MIN_INTERVAL,
                    PROGRESS_AUTO_MAX_INTERVAL,
                )
                or PROGRESS_AUTO_DEFAULT_INTERVAL,
                "auto_last_status": str(cfg.get("auto_last_status", "") or "").strip(),
                "auto_last_message": str(cfg.get("auto_last_message", "") or "").strip(),
                "auto_last_run_at": str(cfg.get("auto_last_run_at", "") or "").strip(),
            }
        ...
        return {
            "paper_subscriptions": {
                ...
                "last_notified_at": paper_last_notified,
            },
            "progress_subscriptions": {
                "total": progress_total,
                "enabled": progress_enabled,
                "last_notified_at": progress_last_notified,
                "by_scope": by_scope,
            },
            "auto_push_health": auto_push_health,
        }

    def _build_monitor_overview(self) -> Dict[str, Any]:
        runtime = self.runner.status() if self.runner else {}
        runtime = runtime if isinstance(runtime, dict) else {}
        paper_job = (
            self.paper_tasks.sync_with_runtime(runtime)
            if self.paper_tasks and isinstance(self.paper_tasks, PaperFetchTaskManager)
            else {}
        )
        paper_job = paper_job if isinstance(paper_job, dict) else {}

        progress_jobs = (
            self.progress_tasks.list_all()
            if self.progress_tasks and isinstance(self.progress_tasks, ProgressFetchTaskManager)
            else {}
        )
        if not isinstance(progress_jobs, dict):
            progress_jobs = {}
        for scope in PROGRESS_SCOPES:
            if not isinstance(progress_jobs.get(scope), dict):
                progress_jobs[scope] = {
                    "scope": scope,
                    "running": False,
                    "started_at": "",
        ...
        schedule_info = self.schedule.current() if self.schedule else {}
        schedule_info = schedule_info if isinstance(schedule_info, dict) else {}

        paper_list_data = {}
        try:
            paper_list_data = self.papers.list_papers(
                limit=1,
                mode="all",
                sort_by="time",
                sort_order="desc",
            ...
            paper_list_data = {"error": f"paper list failed: {type(exc).__name__}: {exc}"}
        paper_list_data = paper_list_data if isinstance(paper_list_data, dict) else {}

        progress_snapshot = self._monitor_collect_progress_snapshot()
        storage = self._monitor_collect_storage()
        push_quality = self._monitor_collect_push_quality()

        paper_result = paper_job.get("result") if isinstance(paper_job.get("result"), dict) else {}
        paper_result = paper_result if isinstance(paper_result, dict) else {}
        paper_duration_seconds = self._monitor_duration_seconds(
            paper_result.get("started_at")
            or paper_job.get("started_at")
            or runtime.get("started_at"),
            paper_result.get("finished_at")
            or paper_job.get("finished_at")
            or runtime.get("finished_at"),
        )

        progress_finished = 0
        progress_success = 0
        scope_errors_tail: List[Dict[str, Any]] = []
        for scope in PROGRESS_SCOPES:
            job = progress_jobs.get(scope) if isinstance(progress_jobs.get(scope), dict) else {}
            result = job.get("result") if isinstance(job.get("result"), dict) else {}
            has_finished = bool(str(job.get("finished_at", "") or "").strip()) or bool(result)
            if has_finished:
                progress_finished += 1
                if bool(result.get("ok")) and not str(job.get("error", "") or "").strip():
                    progress_success += 1
            err_text = str(job.get("error", "") or "").strip() or str(result.get("error", "") or "").strip()
            if err_text:
                scope_errors_tail.append(
                    {
                        "scope": scope,
                        "error": err_text,
        ...
        scope_errors_tail.sort(key=lambda x: str(x.get("finished_at", "") or ""), reverse=True)
        scope_errors_tail = scope_errors_tail[:20]

        risks: List[Dict[str, Any]] = []
        runtime_error = str(runtime.get("last_error", "") or "").strip()
        if runtime_error:
            risks.append({"level": "critical", "scope": "paper", "message": f"paper runtime error: {runtime_error}"})
        if paper_result and paper_result.get("ok") is False:
            risks.append(
                {
                    "level": "critical",
                    "scope": "paper",
                    "message": str(paper_result.get("error", "") or paper_job.get("error", "") or "paper job failed"),
                }
            )
                ...
                risks.append(
                    {
                        "level": "warning",
                        "scope": scope,
                        "message": f"{PROGRESS_SCOPE_NAME_MAP_ZH.get(scope, scope)}: {err_text}",
                    }
                )

        auto_push_health = (
            push_quality.get("auto_push_health", {}) if isinstance(push_quality.get("auto_push_health"), dict) else {}
        )
        now_utc = datetime.now(timezone.utc)
        for scope in PROGRESS_SCOPES:
            row = auto_push_health.get(scope) if isinstance(auto_push_health.get(scope), dict) else {}
            if not bool(row.get("auto_enabled")):
                continue
            if str(row.get("auto_last_status", "") or "").strip().lower() == "error":
                risks.append(
                    {
                        "level": "warning",
                        "scope": scope,
                        "message": f"{PROGRESS_SCOPE_NAME_MAP_ZH.get(scope, scope)} 自动任务最近失败: {row.get('auto_last_message', '')}",
                    }
                )
            last_run = parse_iso_utc(row.get("auto_last_run_at"))
            interval_minutes = parse_int_value(
                row.get("auto_interval_minutes"),
                PROGRESS_AUTO_MIN_INTERVAL,
                PROGRESS_AUTO_MAX_INTERVAL,
            ) or PROGRESS_AUTO_DEFAULT_INTERVAL
            if last_run is not None:
                elapsed = (now_utc - last_run).total_seconds()
                if elapsed > max(3600, interval_minutes * 60 * 2):
                    risks.append(
                        {
                            "level": "warning",
                            "scope": scope,
                            "message": (
                                f"{PROGRESS_SCOPE_NAME_MAP_ZH.get(scope, scope)} 自动任务长时间未运行 "
                                f"({int(elapsed // 60)} 分钟)"
                            ),
                        }
                    )

        for key in ("news_db_latest", "rss_db_latest", "progress_json"):
            info = storage.get(key) if isinstance(storage.get(key), dict) else {}
            age_seconds = info.get("age_seconds")
            if age_seconds is None:
                continue
            if int(age_seconds) > 24 * 3600:
                risks.append(
                    {
                        "level": "warning",
                        "scope": "storage",
                        "message": f"{key} 超过 24 小时未更新",
                    }
                )

        status = "healthy"
        if any(str(x.get("level", "")).lower() == "critical" for x in risks):
            status = "critical"
        elif risks:
            status = "warning"

        running_tasks = int(bool(runtime.get("running")))
        running_tasks += sum(
            1 for scope in PROGRESS_SCOPES if bool((progress_jobs.get(scope) or {}).get("running"))
        )

        progress_success_rate_percent: Optional[float] = None
        if progress_finished > 0:
            progress_success_rate_percent = round((progress_success / progress_finished) * 100.0, 1)

        if isinstance(progress_success_rate_percent, float) and progress_success_rate_percent.is_integer():
            progress_success_rate_percent = int(progress_success_rate_percent)

        progress_last_yield: Dict[str, Dict[str, Any]] = {}
        for scope in PROGRESS_SCOPES:
            job = progress_jobs.get(scope) if isinstance(progress_jobs.get(scope), dict) else {}
            result = job.get("result") if isinstance(job.get("result"), dict) else {}
            progress_last_yield[scope] = {
                "ok": bool(result.get("ok")) if result else False,
                "added": parse_int_value(result.get("added"), 0, 100000000) or 0,
                "updated": parse_int_value(result.get("updated"), 0, 100000000) or 0,
                "scanned": parse_int_value(result.get("scanned"), 0, 100000000) or 0,
                "failed_count": len(result.get("failed", [])) if isinstance(result.get("failed"), list) else 0,
                "message": str(result.get("error", "") or job.get("error", "") or "").strip(),
            }

        papers_total_filtered = parse_int_value(paper_list_data.get("total_filtered"), 0, 100000000) or 0
        papers_db_path = str(paper_list_data.get("db_path", "") or "").strip()
        paper_inventory = self._monitor_collect_paper_inventory(papers_db_path)
        papers_total_items = parse_int_value(paper_inventory.get("total_items"), 0, 100000000) or 0
        papers_source_count = parse_int_value(paper_inventory.get("source_count"), 0, 100000000) or 0
        paper_last_message = str(
            paper_result.get("message", "")
            or paper_result.get("error", "")
            or paper_job.get("error", "")
            or ""
        ).strip()
        paper_progress = runtime.get("progress") if isinstance(runtime.get("progress"), dict) else {}
        paper_fetched = parse_int_value(paper_progress.get("fetched"), 0, 100000000) or 0
        paper_stored = parse_int_value(paper_progress.get("stored"), 0, 100000000) or 0
        paper_analyzed = parse_int_value(paper_progress.get("analyzed"), 0, 100000000) or 0

        runner_logs = runtime.get("logs") if isinstance(runtime.get("logs"), list) else []
        runner_logs_tail = [str(x) for x in runner_logs[-80:]] if isinstance(runner_logs, list) else []
        ...
        return {
            ...
            "kpis": {
                ...
                "paper_last_duration_seconds": paper_duration_seconds,
                "progress_success_rate_percent": progress_success_rate_percent,
            },
            "task_health": {
            ...
            "data_throughput": {
                "papers": {
                    "total_filtered": papers_total_filtered,
                    "total_items": papers_total_items,
                    "db_path": papers_db_path,
                    "source_count": papers_source_count,
                    "last_message": paper_last_message,
                },
                "progress": progress_snapshot,
                "storage": storage,
                "last_run_yield": {
                    "paper": {
                        "ok": bool(paper_result.get("ok")) if paper_result else False,
                        "message": paper_last_message,
                        "scanned": paper_fetched,
                        "added": paper_stored,
                        "updated": paper_analyzed,
                        "exit_code": paper_result.get("exit_code", runtime.get("last_exit_code")),
                        "started_at": str(
                            paper_result.get("started_at", "")
                            or paper_job.get("started_at", "")
                            or runtime.get("started_at", "")
                    ...
                    "progress": progress_last_yield,
                },
            },
            "push_quality": push_quality,
            "risks": risks[:60],
            "live": {
                "runner_logs_tail": runner_logs_tail,
                "scope_errors_tail": scope_errors_tail,
            },
        }
    ...
    def _run_progress_subscription(
        ...
        *,
        ...
        ok, msg = self._send_subscription_notification(
            ...
            settings,
            msg_text,
            subject=f"[OpenHawk] {PROGRESS_SCOPE_NAME_MAP_ZH.get(scope, 'AI资讯')} {subscription_strategy_label(strategy)} 通知",
        )
        result["ok"] = ok
        result["message"] = msg
        if ok:
            self.progress_subscriptions.mark_notified(sub_id, selected_keys, len(items))
        return result
    ...
    def _run_subscription(self, sub: Dict[str, Any]) -> Dict[str, Any]:
        ...
        limit = parse_int_value(settings.get("notify_limit"), 1, 500) or DEFAULT_NOTIFY_LIMIT
        ...
        ok, msg = self._send_subscription_notification(
            ...
            settings,
            msg_text,
            subject=f"[OpenHawk] 论文订阅 {subscription_strategy_label(strategy)} 通知",
        )
        result["ok"] = ok
        result["message"] = msg
        if ok:
            self.subscriptions.mark_notified(sub_id, selected_keys, len(papers))
        return result
    ...
    def _send_test_webhook(
        ...
        now = utc_now_iso()
        text_message = str(message or "").strip() or f"OpenHawk 面板测试通知\n时间: {now}\n渠道: {channel}"
        if channel == "feishu":
            payload = {
                "msg_type": "text",
    ...
    def _send_test_email(
        ...
        msg = MIMEText(
            str(body or "").strip() or f"OpenHawk 面板测试通知\n时间: {utc_now_iso()}\n",
            _subtype="plain",
            _charset="utf-8",
        )
        msg["Subject"] = str(subject or "").strip() or "OpenHawk 测试通知"
        msg["From"] = email_from
    ...
    def do_GET(self) -> None:
        ...
        if path in ("/monitor", "/panel/monitor"):
            self._send_text(self.monitor_html, "text/html; charset=utf-8")
            return
        ...
        if path == "/api/monitor/overview":
            self._send_json(self._build_monitor_overview())
            return
...
def run_server(port: int, output_dir: Path) -> None:
    ...
    DashboardHandler.monitor_html = build_monitor_html()
...
def main() -> None:
    parser = argparse.ArgumentParser(description="OpenHawk paper dashboard server")
