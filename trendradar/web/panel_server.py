# coding=utf-8
"""
Interactive paper dashboard server for trendradar.

Features:
- Manual trigger for `python -m trendradar`
...
from trendradar.web.panel_templates import (
...
class CrawlRunner:
    ...
    def _detect_existing_crawl_pid(self) -> Optional[int]:
        """
        Detect external `python -m trendradar` process to avoid duplicate runs.
        ...
        for entry in proc_root.iterdir():
            ...
            if "trendradar.web.panel_server" in cmdline:
                continue
            if "-m trendradar" in cmdline:
                return pid
...
class ScheduleController:
    ...
    def _build_command_from_settings(self) -> str:
        ...
        if env_parts:
            return f"cd /app && {' '.join(env_parts)} python -m trendradar"
        return "cd /app && python -m trendradar"
